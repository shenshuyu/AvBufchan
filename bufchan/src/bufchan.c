...
long BufChan_Open(char *name, int mode)
{
    ...
    case (AV_RDONLY | AV_CACHED):
        ...
        break;
    case (AV_RDONLY | AV_CACHED | AV_OLDEST):
        ctx->type = AV_STREAM_READER;
        ctx->iter = av_fifo_first_frame(ctx->pool->fifo, 1);
        break;
    case (AV_RDONLY | AV_OLDEST):
        ctx->type = AV_STREAM_READER;
        ctx->iter = av_fifo_first_frame(ctx->pool->fifo, 0);
        break;
    case AV_RDONLY:
        ctx->type = AV_STREAM_READER;
